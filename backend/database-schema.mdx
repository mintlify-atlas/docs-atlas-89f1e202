---
title: "VOZI Supabase Database Schema: Tables and RLS Policies"
...
Each row represents one child profile linked to an adult. The `name` field stores an alias or nickname — it is explicitly **not** a real name. This table has no `updated_at` or `deleted_at` columns; the sync merge rule accounts for this (see [Sync & Auth](/backend/sync-and-auth)).
