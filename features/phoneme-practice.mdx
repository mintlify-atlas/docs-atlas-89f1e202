...
<CardGroup cols={2}>
  ...
  <Card title="Word Card" icon="image">
    A large rounded white card (`_WordCard`) showing the word's image from `assets/words/<word>.png` and the word in 42sp bold text below it. If the image asset doesn't exist, a placeholder icon is shown without crashing.
  </Card>
...
<Steps>
  ...
  <Step title="Play the MP3 asset">
    Calls `_audio.playWord(word.audioKey)` which attempts to play the file at `assets/audio/words/<word>.mp3` — the same audio files used by VOZI iOS.
  </Step>
...
Each word is wrapped in a `PracticeWord` object when loaded. The image asset path is `assets/words/<word>.png` and the audio path is `assets/audio/words/<word>.mp3`.
