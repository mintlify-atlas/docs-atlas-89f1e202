...
**Speak (Hablar):** The app activates the on-device speech recognizer (`speech_to_text`). The microphone listens, transcribes the child's utterance locally, and compares it to the target word using a similarity score. A score of **90% or above** counts as a correct attempt. No audio recording is retained after evaluation — only the resulting pass/fail metric.

When a child passes all 10 words in a phoneme, VOZI plays a session-complete audio cue from `assets/audio/feedback/session/` and displays a confetti celebration driven by an animated Rive character from `assets/rive/`. The child receives a reward that appears in their personal rewards gallery.
...
- **Similarity score** (numeric, 0–100)
- **Pass / fail result**
